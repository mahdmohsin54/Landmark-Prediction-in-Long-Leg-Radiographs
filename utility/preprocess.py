...
def preprocess(image_path_all, image_path, image_segment_path, train_label_txt, train_csv, train_csv_preprocessed, bbox_dict_path):
    # Load the txt file and create a DataFrame
    data = []
    with open(train_label_txt, 'r') as file:
        for line in file:
    ...
    relocate(train_df, image_path_all, image_path, 'train')
    relocate(val_df, image_path_all, image_path, 'validation')

    # Create CSV files
    create_csv(train_df, train_csv)
    create_csv(val_df, train_csv.replace("train", "validation"))
    ...
    train_postprocessed_df = save_segments(train_df, image_path, image_segment_path, bbox_dict, 'train')
    val_postprocessed_df = save_segments(val_df, image_path, image_segment_path, bbox_dict, 'validation')

    # Save postprocessed DataFrames to CSV
    train_postprocessed_df.to_csv(train_csv_preprocessed.replace("preprocessed", "postprocessed"), index=False)
    val_postprocessed_df.to_csv(train_csv_preprocessed.replace("train", "validation").replace("preprocessed", "postprocessed"), index=False)

# Define your paths and parameters
image_path_all = '/home/mahd/image/all'
image_path = '/home/mahd/Label-Augmentation-Folder/image'
image_padded_path = '/home/mahd/Label-Augmentation-Folder/image_padded'
train_label_txt = '/home/mahd/pre_train_EN_cropped.txt'
train_csv = '/home/mahd/Label-Augmentation-Folder/dataframes/train_dataset.csv'
train_csv_preprocessed = '/home/mahd/Label-Augmentation-Folder/dataframes/train_dataset_preprocessed.csv'

bbox_dict_path = '/home/mahd/Long_Leg_Film/New_work/image_to_bboxes_and_landmarks.json'

# Run the preprocess function
preprocess(image_path_all, image_path, image_padded_path, train_label_txt, train_csv, train_csv_preprocessed, bbox_dict_path)