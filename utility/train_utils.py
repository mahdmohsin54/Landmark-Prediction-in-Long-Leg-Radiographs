...
def extract_pixel(args, prediction, num_labels, segment_idx): 
    index_list = []
    box_landmarks = {
        0: [0, 1],
        1: [2],
    ...
    relevant_landmarks = box_landmarks[segment_idx]

    for i in range(len(prediction)):
        tmp_list = []
        for landmark_idx in relevant_landmarks:
            index = (prediction[0][landmark_idx] == torch.max(prediction[0][landmark_idx])).nonzero()
            if len(index) > 1:
...
def rmse(args, prediction, label_list, num_labels, idx, rmse_list, segment_idx, indices):
    index_list = extract_pixel(args, prediction, num_labels, segment_idx)
    if(segment_idx == 0):
        print(f'0: {index_list}')
    if(segment_idx == 1):
        print(f'1: {index_list}')
    index_list = torch.Tensor(np.array(index_list)).squeeze(0).reshape(num_labels*2,1)
    ...
    for i, j in zip(indices, range(num_labels)):
        y = int(label_list[2*i])
        x = int(label_list[2*i+1])

        # print(f'Blah: {index_list[2*j:2*(j+1)]}, {label_list_reshape[2*i:2*(i+1)]}')

        if y != 0 and x != 0:
            rmse_list[i][idx] = mse(index_list[2*j:2*(j+1)], label_list_reshape[2*i:2*(i+1)], squared=False)
        else:
            rmse_list[i][idx] = -1

    return rmse_list, extract_pixel(args, prediction, num_labels, segment_idx)
